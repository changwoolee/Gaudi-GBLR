...
def group_parameters_for_optimizer(model, optimizer_cfg, bias_weight_decay=False,
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            # param_dict.keys()
            if hasattr(m, 'optim_dict') and pn in m.optim_dict:
                p._optim = m.optim_dict[pn]
    ...
    #blacklist_weight_names = ('widths', 'widths_base', 'locations')
    ...
    for pn, p in model.named_parameters():
        if p.requires_grad:
            if pn.split(".")[-1] in blacklist_weight_names:
                param_dict_excluded.append(p)
            else:
                param_dict[pn] = p
    """
    ...
    #if return_excluded:
    #    return param_groups, param_dict_excluded
    #else:
    return param_groups
