...
def train(config: DictConfig) -> Optional[float]:
    ...
    # Resume from the finetuning starting point
    if config.get('resume_ft'):
        #config.trainer.resume_from_checkpoint = str(path)
        model_resume_ft = model.load_from_checkpoint(checkpoint_path=config.resume_ft, strict=True)
        pft_dict = dict(model_resume_ft.model.named_parameters())
        for pn, p in model.model.named_parameters():
            #if not ('bert' in config.resume_ft and 'classifier' in pn):
            if p.size() ==  pft_dict[pn].size():
                p.data = pft_dict[pn].data
        log.info("Resuming from {}".format(config.resume_ft))
