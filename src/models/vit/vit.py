...
def _cfg(url='', input_size=(3, 224, 224), **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': input_size, 'pool_size': None,
...
default_cfgs = {
    'vit_small_patch4_32': _cfg(input_size=(3, 32, 32)),
    'vit_base_patch4_32': _cfg(input_size=(3, 32, 32)),
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 packed_linear=True,
                 sparse_layer_indices=None, # Experimental
                 ):
        """
        Args:
        ...
        if sparse_layer_indices is None:
            sparse_layer_indices = range(depth)
        if isinstance(sparse_layer_indices, int):
            sparse_layer_indices = [sparse_layer_indices]
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                sparsify=i in sparse_layer_indices
                )
...
def vit_small_patch4_32(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(
        patch_size=4, embed_dim=768, depth=8, num_heads=8, **kwargs)
    if pretrained:
        # NOTE my scale was wrong for original weights, leaving this here until I have better ones for this model
        model_kwargs.setdefault('qk_scale', 768 ** -0.5)
    model = _create_vision_transformer('vit_small_patch4_32', pretrained=pretrained, **model_kwargs)
    return model
...
def vit_base_patch4_32(pretrained=False, **kwargs):
    ...
    NOTE:
        ...
        * this model does not have a bias for QKV (unlike the official ViT and DeiT models)
    """
    model_kwargs = dict(patch_size=4, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch4_32', pretrained=pretrained, **model_kwargs)
    return model
