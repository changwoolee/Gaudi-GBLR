        ...
        *function*, not the activation *layer*.
        ...
        if linear1_cfg is None:
            ...
            self.fc1 = hydra.utils.instantiate(linear1_cfg, in_features, hidden_features,
                                               _recursive_=False)
        if act_fn is None:
            if isinstance(act_layer, str) and act_layer.lower() == "identity":
                self.act = nn.Identity()
                logger.info("Using Identity Activation")
            else:
                self.act = act_layer()
        else:
            self.act = act_fn
...
class KernelizedMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., drop_btw_fcs=False, kernel_cfg=None, linear1_cfg=None, act_layer=nn.GELU, act_fn=None, **kwargs):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        ...
        self.kernel = Kernelized(hidden_features, out_features, rank="full", 
